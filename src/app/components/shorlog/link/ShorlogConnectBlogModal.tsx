...
type ShorlogConnectBlogModalProps = {
  isOpen: boolean;
  shorlogId: number;
  onClose: () => void;
  onLinked?: (res: ShorlogBlogLinkResponse) => void;
  onSkip?: () => void;
  showCreateBlogCta?: boolean; // true: 생성 직후, false: 상세에서 연결
...
export default function ShorlogConnectBlogModal({
  ...
  const isAfterPublish = showCreateBlogCta;
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        setLoading(true);
        const list = await fetchMyRecentBlogs(7); // 기본 7개
        if (!cancelled) {
          setRecentBlogs(list);
        }
      } catch (e) {
    ...
    >
      <div className="w-full max-w-[600px] rounded-3xl bg-white shadow-2xl ring-1 ring-black/5">
        ...
        <div className="flex items-start gap-4 px-8 pt-8 pb-6">
          ...
          <div>
            <h2 id="shorlog-connect-blog-title" className="text-lg font-semibold text-slate-900">
              {isAfterPublish ? '숏로그 작성완료!' : '이 숏로그와 연결할 블로그 선택'}
            </h2>
            <p className="mt-2 text-sm text-slate-500">
              {isAfterPublish
                ? '연관된 블로그와 연결하시겠어요?'
                : '내가 쓴 블로그 중에서 이 숏로그와 연결할 항목을 골라 보세요.'}
            </p>
          </div>
        </div>
        ...
        <div className="px-8 py-6">
          ...
          {isAfterPublish && onCreateNewBlog && (
            <button
              type="button"
              onClick={onCreateNewBlog}
              className="flex w-full items-center justify-center rounded-2xl border border-dashed border-emerald-500/70 bg-emerald-50/80 px-5 py-5 text-[15px] font-medium text-emerald-700 shadow-sm transition hover:bg-emerald-100"
            >
              <span className="mr-1.5 text-lg">+</span>
              <span>새 블로그 작성하기</span>
            </button>
          )}
        </div>
        ...
        <div className="flex justify-center gap-3 px-8 py-5">
          <button
            type="button"
            onClick={handleSkip}
            className="inline-flex items-center min-w-[120px] justify-center rounded-full bg-emerald-600 px-4 py-2 text-[14px] font-medium text-white shadow-sm transition hover:bg-emerald-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600"
          >
            나중에 하기
          </button>
        </div>
