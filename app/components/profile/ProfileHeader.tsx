...
import { ProfileInfo } from '@/app/profile/page';

export const ProfileHeader = ({ profile }: { profile: ProfileInfo }) => {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-2xl sm:text-3xl font-bold">{profile.id}</h1>
              <span className="text-sm text-slate-500">{profile.nickname}</span>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-4 text-[13px] text-slate-600">
              ...
              <span>
                <span className="font-semibold">{formatCompactNumber(profile.followerCount)}</span>{' '}
                팔로워
              </span>
              <span>
                <span className="font-semibold">{formatCompactNumber(profile.likeCount)}</span>{' '}
                좋아요
              </span>
            </div>
          </div>
